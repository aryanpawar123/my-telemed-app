...
import { LayoutDashboard, Video, LogOut, Menu, X } from 'lucide-react'; // Removed Settings Icon
    ...
    <>
      ...
      <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="nav-group">
          <div className={`nav-item ${isActive('/dashboard') ? 'active' : ''}`} onClick={() => handleNav('/dashboard')}>
            <LayoutDashboard size={20} /> Dashboard
          </div>
          
          <div className={`nav-item ${isActive('/consultation') ? 'active' : ''}`} onClick={() => handleNav('/consultation')} style={isActive('/consultation') ? { background: '#FFF3E0', color: '#FF5722' } : {}}>
            <Video size={20} /> Consultation
          </div>

          {/* SETTINGS REMOVED HERE */}

          <div className="nav-item" style={{ color: '#D50000', marginTop: 'auto' }} onClick={() => navigate('/')}>
