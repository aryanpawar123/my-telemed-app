...
// --- STEP 1: PATIENT INTAKE (Defined OUTSIDE to fix the typing bug) ---
const Step1 = ({ formData, handleChange, handleToggle, errors, validateStep1, setStep }) => (
    <div className="card">
        ...
        <div className="alert-box alert-blue"><Info size={16} style={{marginTop: '2px', flexShrink: 0}} /><span>Enter Patient ID or Phone Number if the Patient Exists in the Database the Information will Autopopulate if Not a New Patient ID is created</span></div>
        ...
        <div className="alert-box alert-orange"><AlertTriangle size={16} style={{marginTop: '2px', flexShrink: 0}} /><span>Enter Patient ID or Phone Number if the Patient Exists in the Database the Information will Autopopulate</span></div>
        ...
        {[ 
          ...
          { label: 'Showing signs of stroke or heart attack?', key: 'stroke' }
            ...
            <div key={item.key} style={{ background: '#F8F9FA', padding: '15px', borderRadius: '8px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className="alert-box alert-green"><CheckCircle size={16} style={{marginTop: '2px', flexShrink: 0}} /><span>Enter Patient ID or Phone Number if the Patient Exists in the Database the Information will Autopopulate</span></div>
...
// --- STEP 2: WAITING ROOM (Updated Visuals to match image) ---
const Step2 = ({ setStep }) => (
    <div>
       {/* System Checks Grid */}
       <div className="form-grid">
            {/* Card 1: Fixed Device */}
            <div style={{ background: '#E8F5E9', padding: '20px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #C8E6C9' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <div style={{background: '#4CAF50', padding: '10px', borderRadius: '8px', color: 'white'}}><Monitor size={24}/></div>
                    <div>
                        <div style={{fontWeight: 'bold', fontSize: '15px'}}>Fixed Device</div>
                        <div style={{fontSize: '12px', color: '#2E7D32'}}>Dedicated laptop/tablet</div>
                    </div>
                </div>
                <span style={{background: '#4CAF50', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold'}}>✓ Connected</span>
            </div>

            {/* Card 2: Internet */}
            <div style={{ background: '#E8F5E9', padding: '20px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #C8E6C9' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <div style={{background: '#4CAF50', padding: '10px', borderRadius: '8px', color: 'white'}}><Wifi size={24}/></div>
                    <div>
                        <div style={{fontWeight: 'bold', fontSize: '15px'}}>Stable Internet</div>
                        <div style={{fontSize: '12px', color: '#2E7D32'}}>High-speed connection</div>
                    </div>
                </div>
                <span style={{background: '#4CAF50', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold'}}>250 Mbps</span>
            </div>

             {/* Card 3: Camera */}
             <div style={{ background: '#E8F5E9', padding: '20px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #C8E6C9' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <div style={{background: '#4CAF50', padding: '10px', borderRadius: '8px', color: 'white'}}><Camera size={24}/></div>
                    <div>
                        <div style={{fontWeight: 'bold', fontSize: '15px'}}>HD Camera</div>
                        <div style={{fontSize: '12px', color: '#2E7D32'}}>High-quality video feed</div>
                    </div>
                </div>
                <span style={{background: '#4CAF50', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold'}}>✓ Active</span>
            </div>

            {/* Card 4: Audio (Not Ready) */}
             <div style={{ background: '#FFEBEE', padding: '20px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #FFCDD2' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <div style={{background: '#D32F2F', padding: '10px', borderRadius: '8px', color: 'white'}}><Mic size={24}/></div>
                    <div>
                        <div style={{fontWeight: 'bold', fontSize: '15px'}}>Audio System</div>
                        <div style={{fontSize: '12px', color: '#C62828'}}>Clear microphone</div>
                    </div>
                </div>
                <span style={{background: '#D32F2F', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold'}}>✕ Not Ready</span>
            </div>
        </div>

        {/* Setup Complete Section */}
        <div className="card" style={{marginTop: '20px'}}>
            <h3 style={{display: 'flex', alignItems: 'center', gap: '10px', color: '#2E7D32'}}><CheckCircle size={20}/> Setup Complete</h3>
            <ul style={{listStyle: 'none', padding: 0, marginTop: '15px', color: '#555', lineHeight: '2'}}>
                <li style={{display:'flex', gap:'10px', alignItems:'center'}}><span style={{color:'green'}}>•</span> Patient is comfortably seated in a private consultation room</li>
                <li style={{display:'flex', gap:'10px', alignItems:'center'}}><span style={{color:'green'}}>•</span> All equipment has been tested and verified</li>
                <li style={{display:'flex', gap:'10px', alignItems:'center'}}><span style={{color:'green'}}>•</span> No login required - patient access is pre-authorized</li>
                <li style={{display:'flex', gap:'10px', alignItems:'center'}}><span style={{color:'green'}}>•</span> Staff member is available for technical assistance</li>
            </ul>
        </div>

       {/* Patient Ready Bottom Section */}
       <div className="card" style={{ height: '250px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#F3E5F5', border: '1px solid #E1BEE7' }}>
            <div style={{ width: '60px', height: '60px', border: '5px solid white', borderTopColor: '#7B1FA2', borderRadius: '50%', animation: 'spin 1s linear infinite', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}></div>
            <h2 style={{ marginTop: '20px' }}>Patient Ready</h2>
            <p style={{color: '#666'}}>All systems operational. Waiting for doctor to join the consultation...</p>
            <div style={{display:'flex', gap: '5px', marginTop: '10px'}}>
                <span style={{width:'8px', height:'8px', background:'#7B1FA2', borderRadius:'50%'}}></span>
                <span style={{width:'8px', height:'8px', background:'#BA68C8', borderRadius:'50%'}}></span>
                <span style={{width:'8px', height:'8px', background:'#E1BEE7', borderRadius:'50%'}}></span>
            </div>
            <button className="btn btn-primary" style={{marginTop: '20px'}} onClick={() => setStep(3)}>Start Consultation (Demo)</button>
       </div>
    </div>
);
