...
import { FileText, Clock, Video, CheckCircle, Info, AlertTriangle, Download, Mic } from 'lucide-react';
...
const Consultation = () => {
  ...
  const Step1 = () => (
    <div className="card">
        ...
        <div className="form-grid">
            ...
            <div><label>Phone*</label><input name="phone" value={formData.phone} onChange={handleChange} className="input-field" style={errors.phone ? {borderColor:'red'} : {}} />{errors.phone && <small style={{color:'red'}}>{errors.phone}</small>}</div>
        </div>
        ...
        {['chestPain', 'bleeding', 'stroke'].map(k => (
            <div key={k} style={{background:'#F8F9FA', padding:'10px', borderRadius:'5px', marginBottom:'10px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                <span>Severe {k}?</span>
                <div className="toggle-group">
                    <button className={`toggle-btn ${formData[k] === 'No' ? 'active' : ''}`} onClick={() => handleToggle(k, 'No')}>No</button>
                    <button className={`toggle-btn ${formData[k] === 'Yes' ? 'active' : ''}`} onClick={() => handleToggle(k, 'Yes')}>Yes</button>
                </div>
            </div>
        ))}
  ...
  const Step2 = () => (<div className="card" style={{textAlign:'center', padding:'50px'}}><h2>Patient Ready</h2><button className="btn btn-primary" onClick={() => setStep(3)}>Start Consultation</button></div>);
  
  const Step3 = () => (<div className="video-layout"><div className="video-container" style={{display:'flex', alignItems:'center', justifyContent:'center', color:'white'}}>Video Simulator<div style={{position:'absolute', bottom:20}}><button className="btn" style={{background:'red'}} onClick={() => setStep(4)}>End Call</button></div></div></div>);

  const Step4 = () => (
    <div className="card">
        <div className="alert-box alert-green"><CheckCircle size={16}/><span>Consultation Completed</span></div>
        ...
        {showModal && <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center'}}><div className="card" style={{textAlign:'center', width:'300px'}}><h2>Success!</h2><p>Patient saved.</p><button className="btn btn-success" onClick={() => navigate('/dashboard')}>Home</button></div></div>}
    </div>
  );
